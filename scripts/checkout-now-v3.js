...
const CHECKOUT_NEXT_PAGE_SLUG = "2/order/de/eu/upsell1a";
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "p-GsCrbVXExn-tyBRwmnRNhjVEyyjlsrhFVsHKFAFXrBN7EIL-u8HBTdmNOITCXb",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/69fe4e4ba709c6f249d61971-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
...
if (typeof validateAndSendToKlaviyo === "function") {
  try {
    ...
    var pageReadyPayload = {
      ...
      data: { pageName: "VSM - Checkout DE", pageType: "Checkout" },
    };
